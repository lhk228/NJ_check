...
//레이어 이미지 변환
async function convertLayerImage(target) {
    const base64 = await html2canvas(document.querySelector(`#mapDiv${target}`), {
...
//이미지 변환 동기처리
async function handleConvertSync(target) {
    try {
        //맵 변환, 레이어 변환 체크
        const checkCopy = await copyMapToCanvas(target);
        const checkLayer = await convertLayerImage(target);

        READY_CHECK.layer.check = checkLayer;
        READY_CHECK.copyImg.check = checkCopy;

        if (checkLayer && checkCopy) return true;
        else {
